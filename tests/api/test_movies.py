...
class TestMovieApi:
	def test_get_movie_no_params_pagination(self, api_manager:ApiManager, default_movies_pack):
	...
	def test_get_movie_no_params_published_status(self, api_manager:ApiManager, movies_pack_with_status):
		"""Проверка запроса без параметров:	дефолтная выдача published_true"""
		response = api_manager.movies_api.get_movie()
		response_data = response.json()

		for movie in response_data["movies"]:
			assert movie['published'] is True, f"Фильм {movie['name']} статус неопубликован"


	def test_create_and_check_movie(self, api_manager_admin):
	...
	def test_get_movies_filter_by_min_price(self, api_manager:ApiManager, movies_with_various_prices):
		...
		for movie in response_data["movies"]:
			assert movie["price"] >= min_price, f"Фильм {movie['id']} цена меньше {min_price}"

	def test_get_movies_filter_by_max_price(self, api_manager: ApiManager, movies_with_various_prices):
		"""Проверка фильтрации по максмальной цене"""
		max_price = 1000
		...
		for movie in test_movies:
			assert movie["price"] <= max_price, f"Фильм {movie['id']} цена больше {max_price}"

	def test_get_movies_filter_min_max_price(self, api_manager: ApiManager, movies_with_various_prices):
		...
		for movie in test_movies:
			assert min_price <= movie["price"] <= max_price, f"Цена фильма {movie['id']} вне диапазона [{min_price}, {max_price}]"

	def test_get_movies_sort_by_created_at_asc(self, api_manager:ApiManager, movies_with_controlled_dates):
		"""проверка сортировки по возрастания"""
		params = MoviesQweryParams(
			created_at="asc"
		)
		...
		for i in range(len(test_movies) -1):
			current_date = datetime.fromisoformat(test_movies[i].createdAt.replace('Z', '+00:00'))
			next_date = datetime.fromisoformat(test_movies[i + 1].createdAt.replace('Z', '+00:00'))
			assert current_date <= next_date, f"Нет сортировки по возрастанию {current_date} > {next_date}"

	def test_get_movies_sort_by_created_at_desc(self, api_manager:ApiManager, movies_with_controlled_dates):
	...
	def test_post_movie_happy_path(self, api_manager_admin):
	...
	def test_patch_movie_singe_field(self, api_manager_admin, movie_fixture):
		"""обновление одного поля"""
		update_data = {"price": DataGenerator.generate_random_price()}

		response = api_manager_admin.movies_api.patch_movie(movie_fixture['id'], update_data)
	...
	def test_patch_movie_multiple_fields(self, api_manager_admin, movie_fixture):
